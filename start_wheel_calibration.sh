...
docker compose -f ros2_ws/docker-compose.yml run --rm ros2_diff_robot bash -c \
  "source /opt/ros/humble/setup.bash && source /uros_ws/install/local_setup.bash && ros2 run micro_ros_agent micro_ros_agent multiserial --devs \"\$(ls /dev/ttyACM* 2>/dev/null | tr '\n' ' ')\" -b 115200 & sleep 5 && python3 /ros2_ws/helpers/wheel_calibration/calibrate.py"
