 ...
 * @brief High-performance C++ dock pose publisher for Nav2 docking.
 *
...
private:
  void tick()
  {
    ...
    try {
      ...
      // Tag left FOV during close approach — keep publishing last known pose if recent (< 0.8s)
      if (has_pose_) {
        double age_sec = (now - last_detected_time_).seconds();
        if (age_sec < 0.8) {
          last_pose_.header.stamp = now;
          pub_->publish(last_pose_);
        } else {
